"""Print factual mediator probe and direction-patching results."""
...
def _print_table(title: str, table: pd.DataFrame, *, digits: int, max_rows: int) -> None:
    ...
    if table.empty:
        print("NO ROWS")
        return
    print(table.head(int(max_rows)).round(int(digits)).to_string(index=False))
...
def main() -> None:
    ...
    _print_table(
        ...
        _read_csv(analysis_dir / "factual_mediator_summary.csv"),
    ...
    _print_table(
        "FACTUAL CONTROLLED PROJECTION DIAGNOSTICS",
